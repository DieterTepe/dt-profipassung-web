# 🔧 DT-ProfiPassung — Bauplan (plan2.md · AKTUELLER Plan; plan.md = Sicherungskopie im Projektordner)
...
7) MODULKARTE: daten.js (ISO-286-Zahlenkern) · validate.js (Eingabeprüfung) · solver.js
   ...
   Thermik) · rechenweg.js (build/buildFreiform/buildThermik/buildOberflaeche) · schaubild.js (Toleranzfeld-
   SVG: svg + svgGeneral) · beratung.js (Kostenampel/Messmittel/Oberfläche/Schmierspalt) · ui.js (Formular, Modus Passung/Freiform, Thermik-Bereich, Oberfläche-Bereich, i18n,
   Theme, Rechenweg, Grafik) · test_passung.js (Harness, Abschnitte 1–18) · style.css ·
...
DoD: Regel-Tests, alle Hinweise als Codes dreisprachig. — **✓ komplett & am Handy bestätigt (v1.9.3–1.9.5): `beratung.js` (Kostenampel/Verfahren↔IT, Messmittel/10-%-Regel, Oberfläche/Rz, Schmierspalt/Stribeck) + Ergebnis-Panels + selbstprüfender Rechenweg `buildOberflaeche`.**
...
**v1.9.3–1.9.5 (2026-07-16) · B9 Beratungs-Module komplett:** Neues Modul **beratung.js**
(`DTPBeratung`, UMD, DOM-frei, sprachneutrale Codes). Vier Module, alle mit Ergebnis-Panels
(DE/EN/PT) und – wo gerechnet wird – selbstprüfendem Rechenweg:
• **F7 Kostenampel** `costTraffic(IT)` → rot ≤IT6 / gelb IT7–8 / grün ≥IT9 + erreichbare Verfahren
  (Verfahren↔IT-Tabelle).
• **F8 Messmittel** `measurement(T)` → goldene Regel U ≤ T/10 (ideal) bzw. ≤ T/5 (brauchbar),
  Eignungs-Flags je Messmittel inkl. Grenzlehre.
• **F6 Oberfläche** `surface(res,{RzB,RzW})` → Rz-Stufen (T/5, T/3, T/2), Rundheit T/3, wirksames
  Kleinstspiel S_min − 0,4·ΣRz bzw. Kleinstübermaß Ü_min − 0,8·ΣRz (mit Presssitz-Warnung).
• **F9 Schmierspalt** `lubrication(res,{RzB,RzW})` → Stribeck ΣRz ≤ S_min/3, nutzbarer Spalt
  S_min − ΣRz, Mischreibungs-Warnung; nur bei Spielpassungen.
**ui.js**: Beratungs-Panel im Ergebnis (Kostenampel + Messmittel immer; Oberfläche + Schmierspalt
hinter dem neuen Toggle „Oberfläche berücksichtigen (Rz)" mit persistierten Rz-Feldern Bohrung/
Welle); ~44 neue i18n-Keys ×3. **rechenweg.js**: `buildOberflaeche` erzeugt bei aktiver Rz-Sektion
die Rechenweg-Gruppe „Oberfläche & Schmierspalt" mit Formeln + Werten, jeder Schritt per Umkehr-
rechnung selbstgeprüft. **beratung.js** in alle drei HTMLs eingebunden (App vor ui.js, Prüfstand
vor test_passung.js). **test_passung.js**: Abschnitte 16–18 → **Basislinie 146.756 → 150.143**
(+3.387). Verifikation: node --check aller Module · i18n-Parität · DOM-Smokes (SVG + ui.js-Boot)
grün. Kern (daten/validate/solver/freiform/thermik) unverändert. Projektordner + GitHub aktuell.
**Nächster Schritt: B10 Pressverband (DIN 7190).**

**v1.9.6 (2026-07-17) · B10a Laien-ⓘ an allen Eingabefeldern (gebaut & ausgeliefert; Handy-
Bestätigung ausstehend):** Dieters Wunsch: das ⓘ-Prinzip der Schraube (großes rechtsbündiges ⓘ
je Feld → verständliche Sprechblase) VOR B10 auf alle bestehenden Felder ziehen, damit die
Pressverband-Gruppe es von Anfang an nutzt. **ui.js**: neuer Helfer `attachFieldHelp` (help-btn
in der Labelzeile, aufklappbare .field-help-Sprechblase, aria-label/aria-expanded, markTipsSeen);
`labeledField` um 5. Parameter helpKey erweitert; alle 12 Aufrufstellen versorgt (Kurzeingabe,
Nennmaß, System, Bohrung, Welle, Freiform-Nennmaß, ISO-2768-Klasse, Thermik-Temp/-Werkstoffe,
Rz Bohrung/Welle); Bereichs-Schalter Thermik/Oberfläche in neue Flex-Kopfzeile
`.thermik-head-row` gehüllt (ⓘ als Geschwister des <label> → Antippen toggelt NIE die Checkbox);
`applyI18n` zieht neu `[data-i18n-aria]` nach. 14 neue **fh_**-Hilfetexte ×3 Sprachen (Aufbau:
Was ist das? · Bereich · empfohlene Werte mit Begründung; Freiform-Nennmaß mit EIGENEM Text).
**style.css**: `.field-help` (linke Akzentkante, white-space:pre-line), `.thermik-head-row`,
Print blendet help-btn/field-help aus. Verifikation: node --check alle JS · DOM-Smoke
**dom_smoke_b10a.js 16/16** (Boot, 12 ⓘ im Passungs-/2 im Freiform-Modus, Toggle je Knopf,
Checkbox-Stabilität, i18n-Parität über Sprachwechsel DE/EN/PT, Rückschalt-Idempotenz) ·
Harness grün, **Basislinie unverändert 150.143** (reiner UI-Schritt, kein Rechenkern berührt).
daten/validate/solver/freiform/thermik/rechenweg/schaubild/beratung/HTMLs unverändert; geändert
